...
const { notifyProductStatusChange } = require("./notification.service");

const OTP_EXPIRY_MS = 2 * 60 * 1000;

const generateOTP = async (productId, requestingUserId) => {
  ...
  try {
    ...
    if (product.courier_id !== requestingUserId) {
      throw new AppError("You are not the current custodian of this shipment", 403);
    }
...
const verifyOTP = async (productId, otp) => {
  ...
  await notifyProductStatusChange(
    productId,
    'Delivered',
    'Your product has been delivered and verified successfully.'
  );
