...
BEGIN
  ...
  ELSIF NEW.event_type = 'delivered' OR NEW.event_type = 'DELIVERY_COMPLETED' THEN
    change_val := 5;

  ELSIF NEW.event_type = 'OTP_VERIFIED' THEN
    change_val := 2;
...
SELECT * FROM trust_logs ORDER BY created_at DESC;
SELECT id, name, role FROM users;
SELECT name, trust_score FROM users;
SELECT id, name, batch_id, status FROM products;
DELETE FROM products 
WHERE batch_id IN ('BATCH454');
alter type user_role add value if not exists 'customer';
ALTER TABLE products
ADD COLUMN customer_id UUID REFERENCES users(id);
ALTER TABLE products
ADD COLUMN order_hash TEXT,
ADD COLUMN production_hash TEXT,
ADD COLUMN delivery_hash TEXT,
ADD COLUMN integrity_verified BOOLEAN DEFAULT FALSE;
CREATE TYPE requirement_status AS ENUM ('draft', 'pending_approval', 'locked', 'rejected');

CREATE TABLE requirements (
  id                    UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  customer_id           UUID NOT NULL REFERENCES users(id) ON DELETE RESTRICT,
  retailer_id           UUID REFERENCES users(id) ON DELETE RESTRICT,
  manufacturer_id       UUID REFERENCES users(id) ON DELETE RESTRICT,
  title                 TEXT NOT NULL,
  description           TEXT,
  specifications        JSONB NOT NULL DEFAULT '{}'::jsonb,
  status                requirement_status NOT NULL DEFAULT 'draft',
  customer_approved     BOOLEAN NOT NULL DEFAULT FALSE,
  retailer_approved     BOOLEAN NOT NULL DEFAULT FALSE,
  manufacturer_approved BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE INDEX idx_requirements_retailer     ON requirements(retailer_id);
CREATE INDEX idx_requirements_manufacturer ON requirements(manufacturer_id);
